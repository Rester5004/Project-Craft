namespace ProjectCraft.EditorTools
{
    ...
    public static class MachineAliases
    {
        ...
        private static readonly string[,] Table =
        {
            ...
            { "망치", "조합대" },
            { "수전해기", "전기 분해기" },
            // 벽돌 공장 → 압연기 → 압축기 로 두 번 통합됐다. 여기 표는 한 단계만 풀지 않고
            // <b>최종 이름을 직접 가리켜야</b> 한다(ItemAliases 와 같은 규약).
            { "벽돌 공장", "압축기" },
            { "압연기", "압축기" },
            ...
            // '원유 채굴기' 는 '펌프' 와 같은 기계였다 — 둘 다 지형에서 원유 1000 을 뽑는 레시피
            // 하나뿐이었고(oil_extract / extract_oil), 노션도 "원유 채굴기 → 펌프" 로 개명해 뒀다.
            { "원유 채굴기", "펌프" },
